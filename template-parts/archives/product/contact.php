<section class="section-contact pt-10 md:pt-20 xl:pt-16 pb-10 md:pb-24 xl:pb-16 px-4 md:px-8 xl:px-0 overflow-hidden bg-blue-shade-0">
    <div class="theme-container theme-grid fade-in bg-white !pl-4 !pr-11 md:!px-8 xl:!px-9 pt-4 pb-8 md:pt-10 md:pb-10 mb-10 xl:mb-0 rounded-3xl">
        <div class="col-span-2 md:col-span-3 xl:col-span-5 relative">
            <?php
            $contact_title = get_field('products_contact_title', 'option');
            if ($contact_title): ?>
                <h2 class="text-title-h2 text-blue-shade-4 pb-6 md:pb-10 xl:pb-11 invisible fade-in xl:max-w-[350px]">
                    <?php echo $contact_title; ?>
                </h2>
            <?php endif; ?>
            <p class="text-p--2 xl:max-w-[360px] pb-6 md:pb-0 invisible fade-in"><?php the_field('products_contact_text', 'option'); ?></p>
            <?php
            $contact_link = get_field('products_contact_link', 'option');
            if ($contact_link):
                $contact_link_target = $contact_link['target'] ? $contact_link['target'] : '_self';
                ?>
                <a class="btn btn--primary uppercase !px-3 md:!px-7 md:absolute bottom-0 left-0 inline-block" href="<?php echo esc_url($contact_link['url']); ?>"
        ...
        <div class="col-span-1 md:col-span-3 xl:col-span-7 md:flex flex-row justify-end xl:justify-between">
            ...
            if ($image_2):
                echo wp_get_attachment_image($image_2, 'full', false, array('class' => 'w-full h-auto max-w-[265px] rounded-3xl hidden md:block fade-in'));
            endif;
            ?>
        </div>
    </div>
